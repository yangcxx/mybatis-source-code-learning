...
public class XMLConfigBuilder extends BaseBuilder {
  ...
  private XMLConfigBuilder(XPathParser parser, String environment, Properties props) {
    // 初始化 Configuration，主要执行初始化TypeAliasRegistry别名数据；其他很多都只是一个空
  ...
  private void parseConfiguration(XNode root) {
    try {
      ...
      // 多数据库支持：mapper.xml 中SQL指定的话就会被激活
  ...
  private Properties settingsAsProperties(XNode context) {
    ...
    for (Object key : props.keySet()) {
      if (!metaConfig.hasSetter(String.valueOf(key))) {
        throw new BuilderException("The setting " + key + " is not known.  Make sure you spelled it correctly (case " +
          "sensitive).");
      }
    }
    return props;
  ...
  private void settingsElement(Properties props) {
    ...
    configuration.setLazyLoadTriggerMethods(stringSetValueOf(props.getProperty("lazyLoadTriggerMethods"), "equals," +
  ...
  private TransactionFactory transactionManagerElement(XNode context) throws Exception {
    if (context != null) {
      // 事务类型支持：JDBC（使用JDBC的事务），MANAGED（将事务托管给容器）
      String type = context.getStringAttribute("type");
      Properties props = context.getChildrenAsProperties();
  ...
  private void mapperElement(XNode parent) throws Exception {
    if (parent != null) {
      // mappers 支持同时配置 mapper 和 package，package 优先解析
      // 存在重复配置的，package 优先级更高
      for (XNode child : parent.getChildren()) {
        // 涉及到解析注解和 xml 的选择
        if ("package".equals(child.getName())) {
          ...
          if (resource != null && url == null && mapperClass == null) {
            ...
            // 只能拿到对应的 mapper.java，根据相同的地址和文件同名规则进行 mapper.xml 解析
            configuration.addMapper(mapperInterface);
          } else {
            throw new BuilderException("A mapper element may only specify a url, resource or class, but not more than" +
              " one.");
          }
        }
      }
